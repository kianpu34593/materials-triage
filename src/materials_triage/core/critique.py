...
class BoundFlag(BaseModel):
    """An *advisory* concern the critic raised about a hard-constraint bound — one that
    looks inactive (excludes nothing), impossible (excludes everything), or counter to
    the goal. Surfaced to the human, never auto-applied: judging a bound needs
    physical-range knowledge the LLM only approximates, so its opinion is a flag, not a
    correction. A hallucinated flag is then harmless (a note, not a silent rewrite)."""
...
class RankingCritique(BaseModel):
    ...
    (relevance and redundancy, auto-applied via pruning) plus advisory flags on
    constraint bounds (surfaced to the human, not applied)."""
